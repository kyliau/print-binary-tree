...
function printLevel(
  node: AsciiNode | null, x: number, level: number, str: string[]): undefined {
  if (!node) {
    return;
  }
...
export function printAsciiTree(t: Tree | null): string {
  ...
  let str = [];
  // Clone the tree but with each node replaced with an ASCII node
  const root: AsciiNode = buildAsciiTree(t)!;
